...
function Cancel(){
    ...
    for(let i=0; i<orderList.length;i++){
        if(orderList[i].UserId==user.UserId){
            ...
            for(let i=0; i<MedicineList.length;i++){
                alert("what");
                if(MedicineList[i].MedicineId==orderList[i].MedicineId){
                    MedicineList[i].MedicineCount+=orderList[i].MedicineQuantity;
                    user.UserWalletBalance+=orderList[i].MedicinePrice;
    ...
    MedicineList.forEach((item) => {
      ...
      row.innerHTML = `
        ...
        <td>
          ...
          <button onclick="delete(${item.MedicineId})">Delete</button>
...
const cancel = (id: number) => {
    orderList=orderList.filter((item) => item.OrderId!== id);
    alert("remove");
    Cancel();
